...
test "read compressed frame" {
    ...
    ++ "\xf0\x13I am a sentence to be compressed\x2e\x0a" ++ [_]u8{ 0x0f, 0x60, 0x99, 0x2b } // data block checksum
...
test "read two frames" {
    ...
    ++ "This is more than 64 bytes which" ++ [_]u8{ 0x20, 0x00, 0x00, 0x80 } // data block length (32)
    ++ " will make it into three frames " ++ [_]u8{ 0x11, 0x00, 0x00, 0x80 } // data block length (17)
    ++ "with `lz4 -B32`.\x0a" ++ [_]u8{ 0x0, 0x0, 0x0, 0x0 } // end mark
...
test "bad frames don't leak memory" {
    ...
    ++ [_]u8{ 0x64, 0x40, 0xa7 } // frame descriptor
    ++ [_]u8{ 0x20, 0x00, 0x00, 0x80 } // data block length (32)
    ++ "This is something longer than 32 bytes which will cause problems" ++ [_]u8{ 0x0, 0x0, 0x0, 0x0 } // end mark
